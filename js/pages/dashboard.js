(function () {
  ...
  const score = HealthState.getScore();
  const yesterday = HealthState.getScoreYesterday();
  Storage.set("score_yesterday", score);

  document.getElementById("score-value").textContent = score;
  document.getElementById("score-status").textContent = HealthState.getScoreStatus(score);
  ...
  HealthScoreFeature.renderRing(document.getElementById("score-card").querySelector("svg"), score);
  ...
  document.addEventListener("click", function (e) {
    if (!e.target.closest("#score-card")) breakdown.classList.remove("is-visible");
  });
  ...
  const points = trend.map((t, i) => {
    ...
    return { x, y, label: t.day, score: t.score };
  });
  ...
  svg.innerHTML =
    ...
    points.map((p) => '<circle class="trend-chart-dot" cx="' + p.x + '" cy="' + p.y + '" r="4"></circle>').join("") +
    points.map((p) => '<text class="trend-chart-axis" x="' + p.x + '" y="' + (height - 2) + '" text-anchor="middle">' + p.label + "</text>").join("");

  function renderDailyPlan() {
    const plan = MockData.dailyPlan;
    const progress = HealthState.getDailyPlan();
    document.getElementById("daily-plan-list").innerHTML = plan
      .map((p) => {
      ...
      .join("");
    Utils.qsa("[data-plan-toggle]").forEach((btn) => {
  ...
  const recommendations = [
    ...
    { icon: "fa-droplet", title: "Cukupi Kebutuhan Air", desc: "Target hidrasimu masih 1,4 dari 2,0 liter.", href: "health.html" },
